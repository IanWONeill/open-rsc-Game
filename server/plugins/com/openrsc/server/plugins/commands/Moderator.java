...
public final class Moderator implements CommandListener {
    ...
    public GameStateEvent onCommand(String cmd, String[] args, Player player) {
        if (isCommandAllowed(player, cmd)) {
            if (Moderator.messagePrefix == null) {
                Moderator.messagePrefix = player.getWorld().getServer().getConfig().MESSAGE_PREFIX;
            }
            if (Moderator.badSyntaxPrefix == null) {
                Moderator.badSyntaxPrefix = player.getWorld().getServer().getConfig().BAD_SYNTAX_PREFIX;
            }
            ...
            public void init() {
                addState(0, () -> {
                    if (cmd.equalsIgnoreCase("gmute")) {
                        if (args.length < 1) {
                            player.message((Moderator.badSyntaxPrefix + cmd.toUpperCase()) + " [name] (time in minutes, -1 or exclude for permanent)");
                            return null;
                        }
                        Player p = player.getWorld().getPlayer(DataConversions.usernameToHash(args[0]));
                        if (p == null) {
                            player.message(Moderator.messagePrefix + "Invalid name or player is not online");
                            return null;
                        }
                        int minutes = -1;
                        if (args.length >= 2) {
                        ...
                        if ((p.isStaff() && (p.getUsernameHash() != player.getUsernameHash())) && (player.getGroupID() >= p.getGroupID())) {
                            player.message(Moderator.messagePrefix + "You can not mute a staff member of equal or greater rank.");
                            return null;
                        }
                        if (minutes == (-1)) {
                            if (!player.isSuperMod()) {
                                player.message(Moderator.messagePrefix + "You are not allowed to mute indefinitely.");
                                return null;
                            }
                            player.message(((Moderator.messagePrefix + "You have given ") + p.getUsername()) + " a permanent mute from ::g chat.");
                            if (p.getUsernameHash() != player.getUsernameHash()) {
                                p.message(Moderator.messagePrefix + "You have received a permanent mute from (::g) chat.");
                            }
                            p.getCache().store("global_mute", -1);
                        } else {
                            if ((!player.isSuperMod()) && (minutes > 120)) {
                                player.message(Moderator.messagePrefix + "You are not allowed to mute that user for more than 2 hours.");
                                return null;
                            }
                            player.message(((((Moderator.messagePrefix + "You have given ") + p.getUsername()) + " a ") + minutes) + " minute mute from ::g chat.");
                            if (p.getUsernameHash() != player.getUsernameHash()) {
                                p.message(((Moderator.messagePrefix + "You have received a ") + minutes) + " minute mute in (::g) chat.");
                            }
                        ...
                        if (cmd.equalsIgnoreCase("mute")) {
                            if (args.length < 1) {
                                player.message((Moderator.badSyntaxPrefix + cmd.toUpperCase()) + " [name] (time in minutes, -1 for permanent)");
                                return null;
                            }
                            Player p = player.getWorld().getPlayer(DataConversions.usernameToHash(args[0]));
                            if (p == null) {
                            ...
                            if (cmd.equalsIgnoreCase("kick")) {
                                if (args.length < 1) {
                                    player.message((Moderator.badSyntaxPrefix + cmd.toUpperCase()) + " [player]");
                                    return null;
                                }
                                Player p = player.getWorld().getPlayer(DataConversions.usernameToHash(args[0]));
                                if (p == null) {
                                ...
                                if ((p.isStaff() && (p.getUsernameHash() != player.getUsernameHash())) && (player.getGroupID() >= p.getGroupID())) {
                                    player.message(Moderator.messagePrefix + "You can not kick a staff member of equal or greater rank.");
                                    return null;
                                }
                                ...
                                if (cmd.equalsIgnoreCase("alert")) {
                                    ...
                                    if (args.length > 0) {
                                        ...
                                        if (p != null) {
                                            ...
                                            player.message((Moderator.messagePrefix + "Alerted ") + p.getUsername());
                                        } else
                                            player.message(Moderator.messagePrefix + "Invalid name or player is not online");

                                    } else
                                        player.message((Moderator.badSyntaxPrefix + cmd.toUpperCase()) + " [name] [message]");

                                } else
                                    if (cmd.equalsIgnoreCase("summon")) {
                                        if (args.length < 1) {
                                            player.message((Moderator.badSyntaxPrefix + cmd.toUpperCase()) + " [name]");
                                            return null;
                                        }
                                        Player p = player.getWorld().getPlayer(DataConversions.usernameToHash(args[0]));
                                        if (p == null) {
                                        ...
                                        if (p.wasSummoned()) {
                                            player.message(Moderator.messagePrefix + "You can not summon a player who has already been summoned.");
                                            return null;
                                        }
                                        if ((p.isStaff() && (p.getUsernameHash() != player.getUsernameHash())) && (player.getGroupID() >= p.getGroupID())) {
                                            player.message(Moderator.messagePrefix + "You can not summon a staff member of equal or greater rank.");
                                            return null;
                                        }
                                        if (player.getLocation().inWilderness() && (!player.isSuperMod())) {
                                            player.message(Moderator.messagePrefix + "You can not summon players into the wilderness.");
                                            return null;
                                        }
                                        ...
                                        player.message((((((Moderator.messagePrefix + "You have summoned ") + p.getUsername()) + " to ") + p.getLocation()) + " from ") + originalLocation);
                                        if (p.getUsernameHash() != player.getUsernameHash()) {
                                            p.message((Moderator.messagePrefix + "You have been summoned by ") + player.getStaffName());
                                        }
                                    } else
                                        if (cmd.equalsIgnoreCase("say")) {
                                            // SAY is not configged out for mods.
                                            ...
                                            if (cmd.equalsIgnoreCase("info") || cmd.equalsIgnoreCase("about")) {
                                                ...
                                                ActionSender.sendBox(player, (((((((((((((((((((((((((((((((((((("@lre@Player Information: %" + (" %" + "@gre@Name:@whi@ ")) + p.getUsername()) + " %") + "@gre@Group:@whi@ ") + p.getGroupID()) + " %") + "@gre@Fatigue:@whi@ ") + (p.getFatigue() / 750)) + " %") + "@gre@Group ID:@whi@ ") + Group.GROUP_NAMES.get(p.getGroupID())) + " (") + p.getGroupID()) + ") %") + "@gre@Busy:@whi@ ") + (p.isBusy() ? "true" : "false")) + " %") + "@gre@IP:@whi@ ") + p.getLastIP()) + " %") + "@gre@Last Login:@whi@ ") + p.getDaysSinceLastLogin()) + " days ago %") + "@gre@Coordinates:@whi@ ") + p.getStatus()) + " at ") + p.getLocation().toString()) + " %") + "@gre@Last Moved:@whi@ ") + DataConversions.getDateFromMsec(timeMoved)) + " %") + "@gre@Time Logged In:@whi@ ") + DataConversions.getDateFromMsec(timeOnline)) + " %") + "@gre@Total Time Played:@whi@ ") + DataConversions.getDateFromMsec(timePlayed)) + " %", true);
                                            } else
                                                if (cmd.equalsIgnoreCase("inventory")) {
                                                    ...
                                                    if (cmd.equalsIgnoreCase("bank")) {
                                                        ...
                                                        if (((cmd.equalsIgnoreCase("announcement") || cmd.equalsIgnoreCase("announce")) || cmd.equalsIgnoreCase("anouncement")) || cmd.equalsIgnoreCase("anounce")) {
                                                            StringBuilder newStr = new StringBuilder();
                                                            for (String arg : args) {
                                                                newStr.append(arg).append(" ");
                                                            }
