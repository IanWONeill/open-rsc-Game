...
public class Cactus implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    @Override
        ...
        return new GameStateEvent(player.getWorld(), player, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    ...
                    int[] skins = new int[]{ ItemId.WATER_SKIN_MOSTLY_FULL.id(), ItemId.WATER_SKIN_MOSTLY_EMPTY.id(), ItemId.WATER_SKIN_MOUTHFUL_LEFT.id(), ItemId.EMPTY_WATER_SKIN.id() };
                    Functions.showBubble(player, item);
                    player.setBusy(true);
                    player.getWorld().getServer().getGameEventHandler().add(new ShortEvent(player.getWorld(), player, "Cactus Fill Waterskin") {
                        public void action() {
                            for (int s : skins) {
                                if (getOwner().getInventory().remove(s, 1) > (-1)) {
                                    ...
                                    if (s == ItemId.WATER_SKIN_MOSTLY_FULL.id())
                                        newSkin = ItemId.FULL_WATER_SKIN.id();
                                    // More full is one less id number
                                    else
                                        newSkin = s - 1;
                                    // More full is one less id number
                                    ...
                                    getOwner().getWorld().getServer().getGameEventHandler().add(new SingleEvent(getOwner().getWorld(), null, 30000, "Cactus Respawn") {
                                        @Override
                                        public void action() {
                                            if (cacti != null) {
                                ...
                                }// None of this skin in the inventory, try next.
                                 else
                                    continue;
                                // None of this skin in the inventory, try next.
