...
public class SpinningWheel implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    @Override
        ...
        return new GameStateEvent(player.getWorld(), player, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    ...
                    player.setBatchEvent(new BatchEvent(player.getWorld(), player, 600, "Spinning Wheel Batch", Formulae.getRepeatTimes(player, Skills.CRAFTING), false) {
                        @Override
                        public void action() {
                            if (getOwner().getSkills().getLevel(Skills.CRAFTING) < requirement) {
                                Functions.___message(getOwner(), (("You need to have a crafting of level " + requirement) + " or higher to make a ") + new Item(produce).getDef(getWorld()).getName().toLowerCase());
                                interrupt();
                                return;
                            }
                            if (getOwner().getInventory().remove(item.getID(), 1) > (-1)) {
                                ...
                                getOwner().message((((("You " + verb) + " the ") + consumedItem) + " into a ") + producedItem);
