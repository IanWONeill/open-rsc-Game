...
public class DrinkHeating implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    ...
    public boolean blockInvUseOnObject(GameObject obj, Item item, Player p) {
        ...
        if (((item.getID() == ItemId.HALF_COCKTAIL_GLASS.id()) || (item.getID() == ItemId.ODD_LOOKING_COCKTAIL.id())) && (obj.getID() == 119)) {
            return true;
        }
        return false;
    }
    ...
    public GameStateEvent onInvUseOnObject(GameObject obj, Item item, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if ((item.getID() == ItemId.FULL_COCKTAIL_GLASS.id()) && (obj.getID() == 119)) {
                        Functions.___message(p, "you briefly place the drink in the oven");
