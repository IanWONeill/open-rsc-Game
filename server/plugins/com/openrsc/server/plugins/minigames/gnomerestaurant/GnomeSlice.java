...
public class GnomeSlice implements InvUseOnItemListener , InvUseOnItemExecutiveListener {
    ...
    public boolean blockInvUseOnItem(Player p, Item item1, Item item2) {
        if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.ORANGE.id()))
            return true;
        else
            if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.LIME.id()))
                return true;
            else
                if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.PINEAPPLE.id()) || Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.FRESH_PINEAPPLE.id()))
                    return true;
                else
                    if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.LEMON.id()))
                        return true;
            ...
            public void init() {
                addState(0, () -> {
                    if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.ORANGE.id())) {
                        ...
                        int menu = Functions.___showMenu(p, "slice orange", "dice orange");
                        if (menu == 0) {
                        ...
                        if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.LIME.id())) {
                            ...
                            int menu = Functions.___showMenu(p, "slice lime", "dice lime");
                            if (menu == 0) {
                            ...
                            if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.PINEAPPLE.id()) || Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.FRESH_PINEAPPLE.id())) {
                                ...
                                if (Functions.compareItemsIds(item1, item2, ItemId.KNIFE.id(), ItemId.LEMON.id())) {
                                    ...
                                    int menu = Functions.___showMenu(p, "slice lemon", "dice lemon");
                                    if (menu == 0) {
