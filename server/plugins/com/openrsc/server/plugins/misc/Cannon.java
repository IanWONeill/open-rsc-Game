...
public class Cannon implements InvActionListener , InvUseOnObjectListener , ObjectActionListener , InvActionExecutiveListener , InvUseOnObjectExecutiveListener , ObjectActionExecutiveListener {
    public static final int[] cannonObjectIDs = new int[]{ 946// Cannon Base
    , 947// Cannon Stand
    , 948// Cannon Barrels
    , 943// Complete Cannon
     };
    ...
    private void handleBase(Player player, Item item, String command) {
        if (command.equalsIgnoreCase("set down")) {
            ...
            GameObject cannonBase = new GameObject(player.getWorld(), player.getLocation(), Cannon.cannonObjectIDs[0], 0, 0, player.getUsername());
    ...
    private void addCannonStand(Player player, Item item, GameObject object) {
        if ((item.getID() == ItemId.DWARF_CANNON_STAND.id()) && (object.getID() == 946)) {
            ...
            player.getWorld().unregisterGameObject(object);
            GameObject cannonStand = new GameObject(player.getWorld(), object.getLocation(), Cannon.cannonObjectIDs[1], 0, 0, player.getUsername());
    ...
    private void addCannonBarrels(Player player, Item item, GameObject object) {
        if ((item.getID() == ItemId.DWARF_CANNON_BARRELS.id()) && (object.getID() == 947)) {
            ...
            player.getWorld().unregisterGameObject(object);
            GameObject cannonBarrels = new GameObject(player.getWorld(), object.getLocation(), Cannon.cannonObjectIDs[2], 0, 0, player.getUsername());
    ...
    private void addCannonFurnace(Player player, Item item, GameObject object) {
        if ((item.getID() == ItemId.DWARF_CANNON_FURNACE.id()) && (object.getID() == 948)) {
            ...
            player.getWorld().unregisterGameObject(object);
            GameObject cannonFurnace = new GameObject(player.getWorld(), object.getLocation(), Cannon.cannonObjectIDs[3], 0, 0, player.getUsername());
    ...
    public GameStateEvent onObjectAction(GameObject obj, String command, Player player) {
        return new GameStateEvent(player.getWorld(), player, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if (Functions.inArray(obj.getID(), 946, 947, 948, 943) && (!obj.getOwner().equals(player.getUsername()))) {
                        ...
                        if ((!command.equalsIgnoreCase("fire")) && (player.getFatigue() >= player.MAX_FATIGUE))
