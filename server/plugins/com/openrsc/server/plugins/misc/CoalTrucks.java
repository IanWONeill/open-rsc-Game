...
public class CoalTrucks implements InvUseOnObjectListener , ObjectActionListener , InvUseOnObjectExecutiveListener , ObjectActionExecutiveListener {
    ...
    public GameStateEvent onObjectAction(GameObject obj, String command, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if (obj.getID() == CoalTrucks.COAL_TRUCK) {
                        if (p.getCache().hasKey("coal_truck") && (p.getCache().getInt("coal_truck") > 0)) {
                            ...
                            Functions.addItem(p, ItemId.COAL.id(), 1);
    ...
    public boolean blockObjectAction(GameObject obj, String command, Player p) {
        return obj.getID() == CoalTrucks.COAL_TRUCK;
    }

    @Override
    public boolean blockInvUseOnObject(GameObject obj, Item item, Player p) {
        return (obj.getID() == CoalTrucks.COAL_TRUCK) && (item.getID() == ItemId.COAL.id());
    }
    ...
    public GameStateEvent onInvUseOnObject(GameObject obj, Item item, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if ((obj.getID() == CoalTrucks.COAL_TRUCK) && (item.getID() == ItemId.COAL.id())) {
                        p.setBusy(true);
                        ...
                        for (int i = 0; i < coalAmount; i++) {
                            ...
                            Functions.removeItem(p, ItemId.COAL.id(), 1);
                            Functions.sleep(50);
                        }
                        p.setBusy(false);
                    }
