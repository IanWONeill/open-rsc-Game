...
public class CrystalChest implements InvUseOnObjectListener , ObjectActionListener , InvUseOnObjectExecutiveListener , ObjectActionExecutiveListener {
            ...
            public void init() {
                addState(0, () -> {
                    if ((item.getID() == ItemId.CRYSTAL_KEY.id()) && (obj.getID() == CRYSTAL_CHEST)) {
                        ...
                        Functions.replaceObjectDelayed(obj, respawnTime, CRYSTAL_CHEST_OPEN);
                        player.message("You find some treasure in the chest");
                        Functions.removeItem(player, ItemId.CRYSTAL_KEY.id(), 1);// remove the crystal key.
