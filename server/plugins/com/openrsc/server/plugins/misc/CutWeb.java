...
public class CutWeb implements InvUseOnWallObjectListener , WallObjectActionListener , InvUseOnWallObjectExecutiveListener , WallObjectActionExecutiveListener {
    private static int WEB = 24;

    @Override
    public boolean blockInvUseOnWallObject(GameObject obj, Item item, Player p) {
        return obj.getID() == CutWeb.WEB;
    }
    ...
    public boolean blockWallObjectAction(GameObject obj, Integer click, Player p) {
        return p.getWorld().getServer().getConfig().WANT_LEFTCLICK_WEBS && (obj.getID() == CutWeb.WEB);
    }
        ...
        return new GameStateEvent(player.getWorld(), player, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    ...
                    if (canCut) {
                        Functions.___message(player, "You try to destroy the web...");
                        if (Formulae.cutWeb()) {
