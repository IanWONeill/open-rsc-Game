...
public class DragonstoneAmulet implements InvActionListener , InvUseOnObjectListener , InvActionExecutiveListener , InvUseOnObjectExecutiveListener {
    /**
     * RE-CHARGE AMULET
     */
            ...
            public void init() {
                addState(0, () -> {
                    if (item.getID() == ItemId.CHARGED_DRAGONSTONE_AMULET.id()) {
                        ...
                        int menu = Functions.___showMenu(p, "Edgeville", "Karamja", "Draynor village", "Al Kharid", "Nowhere");
                        ...
                        // return;
                        // }
                        if ((((p.getLocation().wildernessLevel() >= Constants.GLORY_TELEPORT_LIMIT) || p.getLocation().isInFisherKingRealm()) || p.getLocation().isInsideGrandTreeGround()) || (p.getLocation().inModRoom() && (!p.isAdmin()))) {
    ...
    public boolean blockInvUseOnObject(GameObject obj, Item item, Player p) {
        return (obj.getID() == DragonstoneAmulet.FOUNTAIN_OF_HEROES) && (item.getID() == ItemId.DRAGONSTONE_AMULET.id());
    }
    ...
    public GameStateEvent onInvUseOnObject(GameObject obj, Item item, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if ((obj.getID() == DragonstoneAmulet.FOUNTAIN_OF_HEROES) && (item.getID() == ItemId.DRAGONSTONE_AMULET.id())) {
                        p.setBusy(true);
                        ...
                        Functions.___message(p, "You feel more power emanating from it than before", "you can now rub this amulet to teleport", "Though using it to much means you will need to recharge it");
