...
public class Hopper implements InvUseOnObjectListener , ObjectActionListener , InvUseOnObjectExecutiveListener , ObjectActionExecutiveListener {
    @Override
        ...
        return new GameStateEvent(player.getWorld(), player, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                ...
                addState(1, () -> {
                    ...
                    if (obj.getID() == 246) {
                        Functions.createGroundItem(getPlayerOwner().getWorld(), ItemId.FLOUR.id(), 1, 162, 3533);
                    } else {
                        Functions.createGroundItem(getPlayerOwner().getWorld(), ItemId.FLOUR.id(), 1, obj.getX(), Formulae.getNewY(Formulae.getNewY(obj.getY(), false), false) + offY);
                    }
