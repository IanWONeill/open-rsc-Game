...
public class Panning implements InvActionListener , InvUseOnNpcListener , InvUseOnObjectListener , ObjectActionListener , InvActionExecutiveListener , InvUseOnNpcExecutiveListener , InvUseOnObjectExecutiveListener , ObjectActionExecutiveListener {
    private static int PANNING_POINT = 1058;

    @Override
    public boolean blockObjectAction(GameObject obj, String command, Player p) {
        return obj.getID() == Panning.PANNING_POINT;
    }
    ...
    public GameStateEvent onObjectAction(GameObject obj, String command, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if (obj.getID() == Panning.PANNING_POINT) {
    ...
    private boolean handlePanning(GameObject obj, Item item, Player p) {
        ...
        p.setBusy(true);
        Functions.showBubble(p, new Item(ItemId.PANNING_TRAY.id()));
        p.playSound("mix");
        p.playerServerMessage(MessageType.QUEST, "You scrape the tray along the bottom");
        Functions.___message(p, "You swirl away the excess water");
        Functions.sleep(1500);
        Functions.showBubble(p, new Item(ItemId.PANNING_TRAY_FULL.id()));
                ...
                addState(0, () -> {
                    if (obj.getID() == Panning.PANNING_POINT) {
                        if (item.getID() == ItemId.PANNING_TRAY.id()) {
                            Npc guide = Functions.getNearestNpc(p, NpcId.DIGSITE_GUIDE.id(), 15);
                            if (guide != null) {
                                // NOT SURE? if(p.getQuestStage(Quests.DIGSITE) < 2) {
                                if (!p.getCache().hasKey("unlocked_panning")) {
                                    Functions.___npcTalk(p, guide, "Hey! you can't pan yet!");
                                    Functions.___playerTalk(p, guide, "Why not ?");
                                    Functions.___npcTalk(p, guide, "We do not allow the uninvited to pan here");
                                    int menu = Functions.___showMenu(p, guide, "Okay, forget it", "So how do I become invited then ?");
                                    if (menu == 0) {
                                        Functions.___npcTalk(p, guide, "You can of course use this place when you know what you are doing");
                                    } else
                                        if (menu == 1) {
                                            Functions.___npcTalk(p, guide, "I'm not supposed to let people pan here", "Unless they have permission from the authorities first", "Mind you I could let you have a go...", "If you're willing to do me a favour");
                                            Functions.___playerTalk(p, guide, "What's that ?");
                                            Functions.___npcTalk(p, guide, "Well...to be honest...", "What I would really like...", "Is a nice cup of tea !");
                                            Functions.___playerTalk(p, guide, "Tea !?");
                                            Functions.___npcTalk(p, guide, "Absolutely, I'm parched !", "If you could bring me one of those...", "I would be more than willing to let you pan here");
                                        }
                ...
                addState(0, () -> {
                    if (npc.getID() == NpcId.DIGSITE_GUIDE.id()) {
                        if (item.getID() == ItemId.PANNING_TRAY.id()) {
                            ...
                            Functions.___npcTalk(p, npc, "Yes, this is a panning tray...");
                        }
                        if (item.getID() == ItemId.PANNING_TRAY_FULL.id()) {
                            p.message("You give the full panning tray to the guide");
                            Functions.___npcTalk(p, npc, "This is no good to me", "I don't deal with finds");
                        }
                        if (item.getID() == ItemId.PANNING_TRAY_GOLD_NUGGET.id()) {
                            p.message("You give the full panning tray to the guide");
                            Functions.___npcTalk(p, npc, "I am afraid I don't deal with finds", "That's not my job");
                        }
                        if (item.getID() == ItemId.CUP_OF_TEA.id()) {
                            if (p.getCache().hasKey("unlocked_panning")) {
                                Functions.___npcTalk(p, npc, "No thanks, I've had enough!");
                            } else {
                                Functions.___npcTalk(p, npc, "Ah! Lovely!", "You can't beat a good cuppa...", "You're free to pan all you want");
                                Functions.___playerTalk(p, npc, "Thanks");
                                Functions.removeItem(p, ItemId.CUP_OF_TEA.id(), 1);
            ...
            public void init() {
                addState(0, () -> {
                    if (item.getID() == ItemId.PANNING_TRAY.id()) {
                        ...
                        Functions.___playerTalk(p, null, "Err, why am I searching an empty tray ?");
                    } else
                        if (item.getID() == ItemId.PANNING_TRAY_FULL.id()) {
                            p.setBusy(true);
                            Functions.___message(p, "You search the contents of the tray...");
                            Functions.sleep(1500);
                                ...
                                if (randomNumber < 50) {
                                    ...
                                    if (randomNumber < 60) {
                                        ...
                                        if (randomNumber < 70) {
                                            ...
                                            int[] randomCoins = new int[]{ 1, 2, 5, 10 };
                            ...
                            if (item.getID() == ItemId.PANNING_TRAY_GOLD_NUGGET.id()) {
                                ...
                                Functions.addItem(p, ItemId.GOLD_NUGGETS.id(), 1);
