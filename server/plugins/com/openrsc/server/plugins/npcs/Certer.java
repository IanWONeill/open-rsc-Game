...
public class Certer implements TalkToNpcListener , TalkToNpcExecutiveListener {
    // official certers
    int[] certers = new int[]{ NpcId.GILES.id(), NpcId.MILES.id(), NpcId.NILES.id(), NpcId.JINNO.id(), NpcId.WATTO.id(), NpcId.OWEN.id(), NpcId.CHUCK.id(), NpcId.ORVEN.id(), NpcId.PADIK.id(), NpcId.SETH.id() };
        ...
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    ...
                    if ((n.getID() == NpcId.FORESTER.id()) && (!p.getWorld().getServer().getConfig().WANT_WOODCUTTING_GUILD)) {
                        return null;
                    }
    ...
    private void beginCertExchange(CerterDef certerDef, Player p, Npc n) {
        Functions.___npcTalk(p, n, ("Welcome to my " + certerDef.getType()) + " exchange stall");
        ...
        switch (firstType) {
            case 0 :
                Functions.___playerTalk(p, n, "I have some certificates to trade in");
                break;
            case 1 :
                Functions.___playerTalk(p, n, (("I have some " + certerDef.getType()) + ending) + " to trade in");
                break;
    ...
    private int firstMenu(CerterDef certerDef, String ending, Player p, Npc n) {
        return Functions.___showMenu(p, n, false, "I have some certificates to trade in", (("I have some " + certerDef.getType()) + ending) + " to trade in", ("What is a " + certerDef.getType()) + " exchange stall?");
    }
    ...
    private int secondMenu(CerterDef certerDef, String ending, Player p, Npc n, int option) {
        ...
        switch (option) {
            case 0 :
                ...
                return Functions.___showMenu(p, n, false, names);
            case 1 :
    ...
    private void decertMenu(CerterDef certerDef, String ending, Player p, Npc n, int index) {
        ...
        if (p.getWorld().getServer().getConfig().WANT_CERTER_BANK_EXCHANGE) {
            certAmount = Functions.___showMenu(p, n, false, "One", "two", "Three", "four", "five", "All to bank");
        } else {
            certAmount = Functions.___showMenu(p, n, false, "One", "two", "Three", "four", "five");
        }
        if (certAmount < 0)
            return;
        ...
        if (certAmount == 5) {
            ...
            if (certAmount <= 0) {
                p.message(("You don't have any " + names[index]) + " certificates");
                return;
            }
            Item bankItem = new Item(itemID, certAmount * 5);
            if (p.getInventory().remove(new Item(certID, certAmount)) > (-1)) {
                p.message(((("You exchange the certificates, " + bankItem.getAmount()) + " ") + bankItem.getDef(p.getWorld()).getName()) + " is added to your bank");
            ...
            if (p.getInventory().remove(certID, certAmount) > (-1)) {
                p.message(("You exchange your certificates for " + certerDef.getType()) + ending);
    ...
    private void certMenu(CerterDef certerDef, String ending, Player p, Npc n, int index) {
        final String[] names = certerDef.getCertNames();
        p.message((("How many " + certerDef.getType()) + ending) + " do you wish to trade in?");
        int certAmount;
        if (p.getWorld().getServer().getConfig().WANT_CERTER_BANK_EXCHANGE) {
            certAmount = Functions.___showMenu(p, n, false, "five", "ten", "Fifteen", "Twenty", "Twentyfive", "All from bank");
        } else {
            certAmount = Functions.___showMenu(p, n, false, "five", "ten", "Fifteen", "Twenty", "Twentyfive");
        }
        if (certAmount < 0)
            return;
        ...
        if (certAmount == 5) {
            ...
            if (p.getBank().remove(itemID, itemAmount) > (-1)) {
                p.message(((((("You exchange the " + certerDef.getType()) + ", ") + itemAmount) + " ") + p.getWorld().getServer().getEntityHandler().getItemDef(itemID).getName()) + " is taken from your bank");
            ...
            if (p.getInventory().countId(itemID) < itemAmount) {
                p.message(((("You don't have that " + (ending.equals("") ? "much" : "many")) + " ") + certerDef.getType()) + ending);
                return;
            }
            p.message((("You exchange your " + certerDef.getType()) + ending) + " for certificates");
            for (int x = 0; x < itemAmount; x++) {
    ...
    private void infMenu(CerterDef certerDef, String ending, Player p, Npc n) {
        ...
        Functions.___playerTalk(p, n, ("What is a " + certerDef.getType()) + " exchange store?");
        Functions.___npcTalk(p, n, ("You may exchange your " + item) + " here", "For certificates which are light and easy to carry", "You can carry many of these certificates at once unlike " + item, ("5 " + item) + " will give you one certificate", ("You may also redeem these certificates here for " + item) + " again", "The advantage of doing this is", ("You can trade large amounts of " + item) + " with other players quickly and safely");
    }
