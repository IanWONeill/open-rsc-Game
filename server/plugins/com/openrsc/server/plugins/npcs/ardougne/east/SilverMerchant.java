...
public class SilverMerchant implements ShopInterface , TalkToNpcListener , TalkToNpcExecutiveListener {
    private final Shop shop = new Shop(false, 60000 * 2, 100, 70, 2, new Item(ItemId.UNSTRUNG_HOLY_SYMBOL_OF_SARADOMIN.id(), 2), new Item(ItemId.SILVER.id(), 1), new Item(ItemId.SILVER_BAR.id(), 1));
            ...
            public void init() {
                addState(0, () -> {
                    if (p.getCache().hasKey("silverStolen") && (Instant.now().getEpochSecond() < (p.getCache().getLong("silverStolen") + 1200))) {
                        ...
                        Npc attacker = Functions.getNearestNpc(p, NpcId.PALADIN.id(), 5);// Paladin first
                        ...
                        Functions.___npcTalk(p, n, "Silver! Silver!", "Best prices for buying and selling in all Kandarin!");
