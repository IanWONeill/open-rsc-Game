...
public final class Barmaid implements TalkToNpcListener , TalkToNpcExecutiveListener {
    ...
    public GameStateEvent onTalkToNpc(Player p, final Npc n) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if ((!p.getCache().hasKey("barcrawl")) && (!p.getCache().hasKey("barthree"))) {
                        NORMAL_ALES(p, n);
                    } else {
                        int barCrawlOpt = // do not send over
                        Functions.___showMenu(p, n, false, "Hi what ales are you serving", "I'm doing Alfred Grimhand's barcrawl");
                        if (barCrawlOpt == 0) {
                            NORMAL_ALES(p, n);
                        } else
                            if (barCrawlOpt == 1) {
                                Functions.___playerTalk(p, n, "I'm doing Alfred Grimhand's barcrawl");
                                Functions.___npcTalk(p, n, "Hehe this'll be fun", "You'll be after our off the menu hand of death cocktail then", "Lots of expensive parts to the cocktail though", "So it will cost you 70 coins");
                                if (Functions.hasItem(p, ItemId.COINS.id(), 70)) {
                                    Functions.___message(p, "You buy a hand of death cocktail");
                                    p.getInventory().remove(ItemId.COINS.id(), 70);
                                    Functions.___message(p, "You drink the cocktail", "You stumble around the room");
                                    drinkAle(p);
                                    p.damage(DataConversions.random(1, 3));
                                    Functions.___message(p, "The barmaid giggles", "The barmaid signs your card");
                                    p.getCache().store("barthree", true);
                                } else {
                                    Functions.___playerTalk(p, n, "I don't have that much money on me");
                                }
                            }
    ...
    private void NORMAL_ALES(Player p, Npc n) {
        Functions.___playerTalk(p, n, "Hi, what ales are you serving?");
        Functions.___npcTalk(p, n, "Well you can either have a nice Asgarnian Ale or a Wizards Mind Bomb", "Or a Dwarven Stout");
        ...
        switch (option) {
            case 0 :
                Functions.___npcTalk(p, n, "That'll be two gold");
                if (Functions.hasItem(p, ItemId.COINS.id(), 2)) {
                    ...
                    Functions.___playerTalk(p, n, notEnoughMoney);
                }
                break;
            case 1 :
                Functions.___npcTalk(p, n, "That'll be two gold");
                ...
                break;
            case 2 :
                Functions.___npcTalk(p, n, "That'll be three gold");
    ...
    private void drinkAle(Player p) {
        int[] skillIDs = new int[]{ Skills.ATTACK, Skills.DEFENSE, Skills.RANGED, Skills.FISHING };
    ...
    private void setAleEffect(Player p, int skillId) {
        ...
        // estimated
        reduction = (maxStat < 15) ? 5 : maxStat < 45 ? 6 : maxStat < 75 ? 7 : 8;
