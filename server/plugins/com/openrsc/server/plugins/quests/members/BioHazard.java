				...
				addState(0, () -> {
					if (n.getID() == NpcId.ELENA_HOUSE.id()) {
						...
						if (p.getQuestStage(quest) == 7) {
							...
							&& hasItem(p, ItemId.SULPHURIC_BROLINE.id()) && hasItem(p, ItemId.ETHENEA.id()) && p.getQuestStage(quest) == 6) {
				...
				addState(0, () -> {
					if (obj.getID() == JERICOS_CUPBOARD_ONE_OPEN || obj.getID() == JERICOS_CUPBOARD_ONE_CLOSED) {
						...
						if (command.equalsIgnoreCase("open")) {
							...
							p.message("you search the cupboard");
							if ((!hasItem(p, ItemId.DOCTORS_GOWN.id())) && (p.getQuestStage(quest) == 4 || p.getQuestStage(quest) == 5)) {
