			...
			public void init() {
				addState(0, () -> {
					if (n.getID() == NpcId.KING_NARNODE_SHAREEN.id()) {
						switch (p.getQuestStage(quest)) {
							case 0:
								...
								if (option == 0) {
									...
									Npc kingNarnode = getNearestNpc(p, NpcId.KING_NARNODE_SHAREEN_UNDERGROUND.id(), 15);
									if (kingNarnode != null) {
										playerTalk(p, kingNarnode, "so what is this place?");
										npcTalk(p, kingNarnode, "these my friend, are the foundations of the stronghold");
										playerTalk(p, kingNarnode, "they just look like roots");
										npcTalk(p, kingNarnode, "not any roots traveller",
											"these were conjured in the past age by gnome mages",
											"since then, they have grown into our mighty stronghold");
										playerTalk(p, kingNarnode, "impressive, but what exactly is the problem?");
										npcTalk(p, kingNarnode, "in the last two months our tree guardians have reported...",
											"...continuing deterioration of the grand trees health",
											"i've never seen this before, it could mean the end for all of us");
										playerTalk(p, kingNarnode, "you mean the tree is ill");
										npcTalk(p, kingNarnode, "in a magical sense yes",
											"would you be willing to help us discover the cause of this illness");
										int op = showMenu(p, kingNarnode, "i'm sorry i don't want to get involved",
											"i'd be happy to help");
										if (op == 0) {
											npcTalk(p, kingNarnode, "i understand traveller",
												"please keep this to yourself");
											playerTalk(p, kingNarnode, "of course");
											npcTalk(p, kingNarnode, "i'll show you the way back up");
											p.message("you follow king shareem up the ladder");
											p.teleport(415, 163);
										} else if (op == 1) {
											npcTalk(p, kingNarnode, "thank guthix for you arrival",
												"the first task is to find out what's killing my tree");
											playerTalk(p, kingNarnode, "have you any ideas?");
											npcTalk(p, kingNarnode,
												"my top tree guardian, glough, believes it's human sabotage",
												"i'm not so sure",
												"the only way to really know, is to talk to Hazelmere");
											playerTalk(p, kingNarnode, "who's hazelmere?");
											npcTalk(p, kingNarnode, "a once all powerful mage who created the grand tree",
											...
											playerTalk(p, kingNarnode, "what is it?");
											npcTalk(p, kingNarnode, "a translation book, translate carefully, his words may save us all",
						...
						} else if (p.getQuestStage(quest) == 9) {
							Npc HUT_FOREMAN = getNearestNpc(p, NpcId.SHIPYARD_FOREMAN_HUT.id(), 4);
		...
		return new GameStateEvent(p.getWorld(), p, 0, getClass().getSimpleName() + " " + getClass().getEnclosingMethod().getName()) {
			public void init() {
				addState(0, () -> {
					...
					else if (affectedmob.getID() == NpcId.SHIPYARD_FOREMAN_HUT.id() && p.getQuestStage(quest) == 10) {
						p.message("the forman is too busy to talk");
					}
