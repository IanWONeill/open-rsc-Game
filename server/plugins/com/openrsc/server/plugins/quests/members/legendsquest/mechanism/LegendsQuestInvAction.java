...
public class LegendsQuestInvAction implements InvActionListener , InvUseOnItemListener , InvActionExecutiveListener , InvUseOnItemExecutiveListener {
    @Override
    public boolean blockInvAction(Item item, Player p, String command) {
        return Functions.inArray(item.getID(), ItemId.SCRIBBLED_NOTES.id(), ItemId.SCRAWLED_NOTES.id(), ItemId.SCATCHED_NOTES.id(), ItemId.ROUGH_SKETCH_OF_A_BOWL.id(), ItemId.SHAMANS_TOME.id(), ItemId.BOOKING_OF_BINDING.id(), ItemId.YOMMI_TREE_SEED.id(), ItemId.GERMINATED_YOMMI_TREE_SEED.id(), ItemId.A_RED_CRYSTAL.id(), ItemId.HOLY_FORCE_SPELL.id(), ItemId.GILDED_TOTEM_POLE.id());
    }
            ...
            public void init() {
                addState(0, () -> {
                    if (item.getID() == ItemId.GILDED_TOTEM_POLE.id()) {
                        Functions.___message(p, 1300, "This totem pole is utterly awe inspiring.", "Perhaps you should show it to Radimus Erkle...");
                    } else
                        if (item.getID() == ItemId.HOLY_FORCE_SPELL.id()) {
                            Npc n = Functions.getNearestNpc(p, NpcId.ECHNED_ZEKIN.id(), 5);
                            if ((n != null) && (p.getQuestStage(Quests.LEGENDS_QUEST) == 7)) {
                                ...
                                Functions.___message(p, n, 1300, "The spirit lets out an unearthly, blood curdling scream...");
                                ...
                                Functions.___message(p, 600, "There is no suitable candidate to cast this spell on.");
                            }
                        } else
                            if ((item.getID() == ItemId.A_RED_CRYSTAL.id()) && (!p.getWorld().getServer().getConfig().WANT_PETS)) {
                                Functions.___message(p, 1300, "As the crystal touches your hands a voice inside of your head says..", "@gre@Voice in head: Bring life to the dragons eye.");
                            } else
                                if (item.getID() == ItemId.GERMINATED_YOMMI_TREE_SEED.id()) {
                                    Functions.___message(p, 1300, "These seeds have been germinated in pure water...");
                                    p.message("They can be planted in fertile soil now...");
                                } else
                                    if (item.getID() == ItemId.YOMMI_TREE_SEED.id()) {
                                        p.message("These seeds need to be germinated in pure water...");
                                    } else
                                        if (item.getID() == ItemId.BOOKING_OF_BINDING.id()) {
                                            p.message("You read the Book of Binding...");
                                            int page = Functions.___showMenu(p, "Arcana..", "Instructo...", "Defeati...", "Enchanto...");
                                            if (page == 0) {
                                                Functions.___message(p, 1300, "You read the section entitled Arcana...");
                                                ...
                                                if (page == 1) {
                                                    Functions.___message(p, 1300, "You read the section entitled Instructo...");
                                                    ...
                                                    if (page == 2) {
                                                        Functions.___message(p, 1300, "You read the section entitled Defeati...");
                                                        ...
                                                        if (page == 3) {
                                                            Functions.___message(p, 1300, "You read the section entitled Enchanto...", "This looks like an enchantment, it requires some magic and prayer to cast.");
                                                            p.message("Would you like to try and cast this enchantment?");
                                                            int opt = Functions.___showMenu(p, "Yes, I'll try.", "No, I don't think I'll bother.");
                                                            ...
                                                            if ((opt == 0) || (opt == 1)) {
                                                                ...
                                                                if (Functions.getCurrentLevel(p, Skills.MAGIC) < 10) {
                                                                    p.message("You need at least ten magic points to cast this spell.");
                                                                    return null;
                                                                }
                                                                if (Functions.hasItem(p, ItemId.EMPTY_VIAL.id())) {
                                                                    Functions.___message(p, "The spell is cast perfectly..", "You enchant one of the empty vials.");
                                            ...
                                            if (Functions.inArray(item.getID(), ItemId.SCRIBBLED_NOTES.id(), ItemId.SCRAWLED_NOTES.id(), ItemId.SCATCHED_NOTES.id())) {
                                                ...
                                                if (item.getID() == ItemId.SCRIBBLED_NOTES.id()) {
                                                    ...
                                                    if (item.getID() == ItemId.SCRAWLED_NOTES.id()) {
                                                        ...
                                                        if (item.getID() == ItemId.SCATCHED_NOTES.id()) {
                                                ...
                                                if (item.getID() == ItemId.ROUGH_SKETCH_OF_A_BOWL.id()) {
                                                    Functions.___message(p, 1300, "You look at the rough sketch that Gujuo gave you.");
                                                    p.message("It looks like a picture of a bowl...");
                                                } else
                                                    if (item.getID() == ItemId.SHAMANS_TOME.id()) {
                                                        Functions.___message(p, 1300, "You read the ancient shamans tome.");
                                                        Functions.___message(p, 3800, "It is written in a strange sort of language but you manage a rough translation.");
                ...
                addState(0, () -> {
                    if (Functions.compareItemsIds(item1, item2, ItemId.YOMMI_TREE_SEED.id(), ItemId.BLESSED_GOLDEN_BOWL_WITH_PURE_WATER.id())) {
                        for (int i = 0; i < p.getInventory().countId(ItemId.YOMMI_TREE_SEED.id()); i++) {
                            Functions.removeItem(p, ItemId.YOMMI_TREE_SEED.id(), 1);
                            Functions.addItem(p, ItemId.GERMINATED_YOMMI_TREE_SEED.id(), 1);
                        }
                        ...
                        Functions.___message(p, 1300, "The pure water in the golden bowl has run out...");
