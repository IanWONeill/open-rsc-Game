...
public class LegendsQuestSanTojalon implements PlayerAttackNpcListener , PlayerKilledNpcListener , PlayerMageNpcListener , PlayerNpcRunListener , PlayerRangeNpcListener , PlayerAttackNpcExecutiveListener , PlayerKilledNpcExecutiveListener , PlayerMageNpcExecutiveListener , PlayerNpcRunExecutiveListener , PlayerRangeNpcExecutiveListener {
    @Override
    public boolean blockPlayerAttackNpc(Player p, Npc n) {
        return ((n.getID() == NpcId.SAN_TOJALON.id()) && (!Functions.hasItem(p, ItemId.A_CHUNK_OF_CRYSTAL.id()))) && (!p.getCache().hasKey("cavernous_opening"));
    }
    ...
    public GameStateEvent onPlayerAttackNpc(Player p, Npc n) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if (((n.getID() == NpcId.SAN_TOJALON.id()) && (!Functions.hasItem(p, ItemId.A_CHUNK_OF_CRYSTAL.id()))) && (!p.getCache().hasKey("cavernous_opening"))) {
                        attackMessage(p, n);
                    }
                    return null;
                });
    ...
    private void attackMessage(Player p, Npc n) {
        if (((n.getID() == NpcId.SAN_TOJALON.id()) && (!Functions.hasItem(p, ItemId.A_CHUNK_OF_CRYSTAL.id()))) && (!p.getCache().hasKey("cavernous_opening"))) {
            Functions.___npcTalk(p, n, "You have entered the Viyeldi caves and  your bravery must be tested.");
            n.setChasing(p);
            Functions.___npcTalk(p, n, "Prepare yourself...San Tojalon will test your mettle.");
        }
    }

    @Override
    public boolean blockPlayerKilledNpc(Player p, Npc n) {
        return ((n.getID() == NpcId.SAN_TOJALON.id()) && (!p.getCache().hasKey("cavernous_opening"))) || (((n.getID() == NpcId.SAN_TOJALON.id()) && (p.getQuestStage(Quests.LEGENDS_QUEST) == 8)) && p.getCache().hasKey("viyeldi_companions"));
    }
            ...
            public void init() {
                addState(0, () -> {
                    if (((n.getID() == NpcId.SAN_TOJALON.id()) && (p.getQuestStage(Quests.LEGENDS_QUEST) == 8)) && p.getCache().hasKey("viyeldi_companions")) {
                        ...
                        Functions.___message(p, 1300, "A nerve tingling scream echoes around you as you slay the dead Hero.", "@yel@San Tojalon: Ahhhggggh", "@yel@San Tojalon: Forever must I live in this torment till this beast is slain...");
                        Functions.sleep(650);
                        LegendsQuestNezikchened.demonFight(p);
                    }
                    if ((n.getID() == NpcId.SAN_TOJALON.id()) && (!p.getCache().hasKey("cavernous_opening"))) {
                        if ((Functions.hasItem(p, ItemId.A_CHUNK_OF_CRYSTAL.id()) || Functions.hasItem(p, ItemId.A_RED_CRYSTAL.id())) || Functions.hasItem(p, ItemId.A_GLOWING_RED_CRYSTAL.id())) {
                            ...
                            Functions.addItem(p, ItemId.A_CHUNK_OF_CRYSTAL.id(), 1);
                        }
                    }
                    return null;
                });
