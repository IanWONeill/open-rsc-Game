...
public class UndergroundPassIban implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    ...
    public boolean blockInvUseOnObject(GameObject obj, Item item, Player p) {
        return (obj.getID() == UndergroundPassIban.PIT_OF_THE_DAMNED) && (item.getID() == ItemId.A_DOLL_OF_IBAN.id());
    }
    ...
    public GameStateEvent onInvUseOnObject(GameObject obj, Item item, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if (obj.getID() == UndergroundPassIban.PIT_OF_THE_DAMNED) {
                        // iban has been killed (+ doesn't matter status of doll)
                        if (Functions.atQuestStages(p, Quests.UNDERGROUND_PASS, 8, -1)) {
                            p.message("iban is already dead");
                            return null;
                        }
                        boolean defeated = false;
                        if (((p.getCache().hasKey("poison_on_doll") && p.getCache().hasKey("cons_on_doll")) && p.getCache().hasKey("ash_on_doll")) && p.getCache().hasKey("shadow_on_doll")) {
                            Npc iban = Functions.getNearestNpc(p, NpcId.IBAN.id(), 10);
                            if (iban == null) {
                                p.message("iban is still not here");
                                return null;
                            } else {
                                Functions.___message(p, "you throw the doll of iban into the pit");
                                Functions.removeItem(p, new Item(ItemId.A_DOLL_OF_IBAN.id(), 1));
                                defeated = true;
                                p.setAttribute("iban_bubble_show", true);
                                Functions.___npcTalk(p, iban, "what's happening?, it's dark here...so dark", "im falling into the dark, what have you done?");
                                Functions.___message(p, "iban falls to his knees clutching his throat");
                                Functions.___npcTalk(p, iban, "noooooooo!");
                                p.message("iban slumps motionless to the floor");
                                iban.remove();
                            }
                            if (defeated) {
                                Functions.___message(p, "a roar comes from the pit of the damned", "the infamous iban has finally gone to rest");
                                p.message("amongst ibans remains you find his staff..");
                                Functions.___message(p, "...and some runes");
                                p.message("suddenly around you rocks crash to the floor..");
                                Functions.___message(p, "...as the ground begins to shake", "the temple walls begin to collapse in", "and you're thrown from the temple platform");
                                Functions.addItem(p, ItemId.STAFF_OF_IBAN.id(), 1);
                                Functions.addItem(p, ItemId.DEATH_RUNE.id(), 15);
                                Functions.addItem(p, ItemId.FIRE_RUNE.id(), 30);
                                p.teleport(687, 3485);
