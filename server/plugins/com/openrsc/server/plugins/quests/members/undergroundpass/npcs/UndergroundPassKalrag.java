            ...
            public void init() {
                addState(0, () -> {
                    if (n.getID() == NpcId.KALRAG.id()) {
                        n.killedBy(p);
                        Functions.___message(p, "kalrag slumps to the floor", "poison flows from the corpse over the soil");
                        if ((!p.getCache().hasKey("poison_on_doll")) && (p.getQuestStage(Quests.UNDERGROUND_PASS) == 6)) {
                            if (Functions.hasItem(p, ItemId.A_DOLL_OF_IBAN.id())) {
                                Functions.___message(p, "you smear the doll of iban in the poisoned blood");
                                p.message("it smells horrific");
                                p.getCache().store("poison_on_doll", true);
                            } else {
                                Functions.___message(p, "it quikly seeps away into the earth");
