	...
	public GameStateEvent onObjectAction(GameObject obj, String command, Player p) {
		return new GameStateEvent(p.getWorld(), p, 0, getClass().getSimpleName() + " " + Thread.currentThread().getStackTrace()[1].getMethodName()) {
			public void init() {
				addState(0, () -> {
					if (inArray(obj.getID(), PILE_OF_MUD_MAP_LEVEL_2)) {
						if (obj.getID() == PILE_OF_MUD_MAP_LEVEL_2[0]) {
							return invokeOnNotify(questMessage(p, "you climb the pile of mud...",
								"it leads to a small tunnel..."), 1);
						}
						else if (obj.getID() == PILE_OF_MUD_MAP_LEVEL_2[1]) {
							return invokeOnNotify(questMessage(p, "you climb the pile of mud"), 2);
						}
						else if (obj.getID() == PILE_OF_MUD_MAP_LEVEL_2[2]) {
							return invokeOnNotify(questMessage(p, "you climb the pile of mud"), 3);
						}
						else if (obj.getID() == PILE_OF_MUD_MAP_LEVEL_2[3]) {
							return invokeOnNotify(questMessage(p, "you climb the pile of mud"), 4);
						}
						else if (obj.getID() == PILE_OF_MUD_MAP_LEVEL_2[4]) {
							return invokeOnNotify(questMessage(p, "you climb the pile of mud"), 5);
						}
						else if (obj.getID() == PILE_OF_MUD_MAP_LEVEL_2[5]) {
							return invokeOnNotify(questMessage(p, "you climb the pile of mud"), 6);
						}
					}
					else if (obj.getID() == CRATE) {
						return invokeOnNotify(questMessage(p, "you search the crate"), 7);
					}
					else if (inArray(obj.getID(), DUG_UP_SOIL)) {
						return invokeOnNotify(questMessage(p, "under the soil is a tunnel"), 8);
					}
						...
						if (!p.getCache().hasKey("rope_wall_grill")) {
							return endOnNotify(questMessage(p, "the wall grill is too high", "you can't quite reach"));
						} else {
							return invokeOnNotify(questMessage(p, "you use the rope tied to the grill to pull yourself up", "you then climb across the grill to the otherside"), 12);
						}
					}
					else if (obj.getID() == WALL_GRILL_WEST) {
						return invokeOnNotify(questMessage(p, "you climb across the grill to the otherside"), 13);
					}
					else if (inArray(obj.getID(), ROCKS)) {
						switch (obj.getID()) {
							case 859:
							case 858:
								return endOnNotify(UndergroundPassObstaclesMap1.doRock(obj, p, (int) (getCurrentLevel(p, Skills.HITS) / 5) + 5, false, 5)); // fall side 5.
							case 854:
							case 853:
							case 855:
							case 857:
								return endOnNotify(UndergroundPassObstaclesMap1.doRock(obj, p, (int) (getCurrentLevel(p, Skills.HITS) / 5) + 5, false, 4)); // fall side 4.
							case 852:
								return endOnNotify(UndergroundPassObstaclesMap1.doRock(obj, p, (int) (getCurrentLevel(p, Skills.HITS) / 5) + 5, false, 3)); // fall side 3.
							case 851:
								return endOnNotify(UndergroundPassObstaclesMap1.doRock(obj, p, (int) (getCurrentLevel(p, Skills.HITS) / 5) + 5, false, 2)); // fall side 2.
							default:
								return endOnNotify(UndergroundPassObstaclesMap1.doRock(obj, p, (int) (getCurrentLevel(p, Skills.HITS) / 5) + 5, false, 1)); // fall side 1.
						}
					}
						...
						return invokeOnNotify(questMessage(p, "you walk down the passage way"), 14);
					}
					else if (obj.getID() == CAGE_REMAINS) {
						if (p.getQuestStage(Quests.UNDERGROUND_PASS) >= 5 || p.getQuestStage(Quests.UNDERGROUND_PASS) == -1) {
							return endOnNotify(questMessage(p, "you search the cage remains", "nothing remains"));
						}
						...
						if ((p.getCache().hasKey("flames_of_zamorak1") && p.getCache().hasKey("flames_of_zamorak2") && (p.getCache().hasKey("flames_of_zamorak3") && p.getCache().getInt("flames_of_zamorak3") >= 2)) || atQuestStages(p, Quests.UNDERGROUND_PASS, 7, 8, -1)) {
							return invokeOnNotify(questMessage(p, "from behind the door you hear cry's and moans"), 16);
						} else {
						...
						p.teleport(766, 3417);
						return invokeOnNotify(questMessage(p, "you open the huge wooden door"), 18);
					}
					else if (obj.getID() == FLAMES_OF_ZAMORAK) {
						return invokeOnNotify(questMessage(p, "you search the stone structure"), 19);
					}
				...
				addState(1, () -> {
					p.teleport(727, 3448);
					return endOnNotify(questMessage(p, "..ending at the well entrance"));
				});
				addState(2, () -> {
					p.teleport(753, 3481);
					return null;
				...
				addState(8, () -> {
					...
					return invokeOnNotify(showPlayerMenu(getPlayerOwner(), "no, im scared of small spaces", "yep, let's do it"), 9);
				});
				addState(9, () -> {
					final int menu = (int)getNotifyEvent().getObjectOut("int_option");
					if (menu == 1) {
						return invokeOnNotify(questMessage(p, "you climb into the small tunnel"), 10);
					}
					return null;
				});
				addState(10, () -> {
				...
				addState(11, () -> {
					...
					p.damage((int) (getCurrentLevel(p, Skills.HITS) / 5) + 5);
					p.teleport(764, 3467);
					return endOnNotify(playerDialogue(p, null, "ooof"));
				});
				addState(12, () -> {
					p.teleport(762, 3472);
					return null;
				...
				addState(14, () -> {
					...
					p.damage((int) (getCurrentLevel(p, Skills.HITS) / 5) + 5);
					return endOnNotify(playerDialogue(p, null, "aaarghh"));
				});
				addState(15, () -> {
			...
			public void init() {
				addState(0, () -> {
					if (inArray(obj.getID(), RAILINGS)) {
						if (click == 0) {
							if (obj.getID() == 168) {
								return endOnNotify(questMessage(getPlayerOwner(), "the cage door has been sealed shut", "the poor unicorn can't escape"));
							}
							...
							if (obj.getID() == 168) {
								return invokeOnNotify(questMessage(getPlayerOwner(), "you search the cage"), 2);
							}
