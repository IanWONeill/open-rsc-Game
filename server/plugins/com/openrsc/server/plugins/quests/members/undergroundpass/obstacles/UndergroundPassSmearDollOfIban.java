...
public class UndergroundPassSmearDollOfIban implements InvUseOnItemListener , InvUseOnItemExecutiveListener {
    ...
    public boolean blockInvUseOnItem(Player p, Item item1, Item item2) {
        return (Functions.compareItemsIds(item1, item2, ItemId.IBANS_ASHES.id(), ItemId.A_DOLL_OF_IBAN.id()) || Functions.compareItemsIds(item1, item2, ItemId.IBANS_CONSCIENCE.id(), ItemId.A_DOLL_OF_IBAN.id())) || Functions.compareItemsIds(item1, item2, ItemId.IBANS_SHADOW.id(), ItemId.A_DOLL_OF_IBAN.id());
    }
            ...
            public void init() {
                addState(0, () -> {
                    if (Functions.compareItemsIds(item1, item2, ItemId.IBANS_ASHES.id(), ItemId.A_DOLL_OF_IBAN.id())) {
                        ...
                        Functions.removeItem(p, ItemId.IBANS_ASHES.id(), 1);
                        if ((!p.getCache().hasKey("ash_on_doll")) && (p.getQuestStage(Quests.UNDERGROUND_PASS) == 6)) {
                            p.getCache().store("ash_on_doll", true);
                        }
                    } else
                        if (Functions.compareItemsIds(item1, item2, ItemId.IBANS_CONSCIENCE.id(), ItemId.A_DOLL_OF_IBAN.id())) {
                            Functions.___message(p, "you crumble the doves skeleton into dust");
                            p.message("and rub it into the doll");
                            Functions.removeItem(p, ItemId.IBANS_CONSCIENCE.id(), 1);
                            if ((!p.getCache().hasKey("cons_on_doll")) && (p.getQuestStage(Quests.UNDERGROUND_PASS) == 6)) {
                                p.getCache().store("cons_on_doll", true);
                            }
                        } else
                            if (Functions.compareItemsIds(item1, item2, ItemId.IBANS_SHADOW.id(), ItemId.A_DOLL_OF_IBAN.id())) {
                                Functions.___message(p, "you pour the strange liquid over the doll");
                                p.message("it seeps into the cotton");
                                Functions.removeItem(p, ItemId.IBANS_SHADOW.id(), 1);
