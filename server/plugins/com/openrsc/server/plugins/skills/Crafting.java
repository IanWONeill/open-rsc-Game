...
public class Crafting implements InvUseOnItemListener , InvUseOnObjectListener , InvUseOnItemExecutiveListener , InvUseOnObjectExecutiveListener {
    /**
     * World instance
     */
    private int[] gemsThatFail = new int[]{ ItemId.UNCUT_RED_TOPAZ.id(), ItemId.UNCUT_JADE.id(), ItemId.UNCUT_OPAL.id() };

    private int[] itemsFurnance = new int[]{ ItemId.SILVER_BAR.id(), ItemId.GOLD_BAR.id(), ItemId.SODA_ASH.id(), ItemId.SAND.id(), ItemId.GOLD_BAR_FAMILYCREST.id() };

    private int[] itemsOven = new int[]{ ItemId.UNFIRED_POT.id(), ItemId.UNFIRED_PIE_DISH.id(), ItemId.UNFIRED_BOWL.id() };
            ...
            public void init() {
                addState(0, () -> {
                    if (item1.getID() == ItemId.CHISEL.id()) {
                        ...
                        if (item2.getID() == ItemId.CHISEL.id()) {
                            doCutGem(player, item2, item1);
                        } else
                            if (item1.getID() == ItemId.GLASSBLOWING_PIPE.id()) {
                                doGlassBlowing(player, item1, item2);
                            } else
                                if (item2.getID() == ItemId.GLASSBLOWING_PIPE.id()) {
                                    doGlassBlowing(player, item2, item1);
                                } else
                                    if (item1.getID() == ItemId.NEEDLE.id()) {
                                        makeLeather(player, item1, item2);
                                    } else
                                        if (item2.getID() == ItemId.NEEDLE.id()) {
                                            makeLeather(player, item2, item1);
                                        } else
                                            if (item1.getID() == ItemId.BALL_OF_WOOL.id()) {
                                                useWool(player, item1, item2);
                                            } else
                                                if (item2.getID() == ItemId.BALL_OF_WOOL.id()) {
                                                    useWool(player, item2, item1);
                                                } else
                                                    if ((((item1.getID() == ItemId.BUCKET_OF_WATER.id()) || (item1.getID() == ItemId.JUG_OF_WATER.id())) || (item1.getID() == ItemId.BOWL_OF_WATER.id())) && useWater(player, item1, item2)) {
                                                        return null;
                                                    } else
                                                        if ((((item2.getID() == ItemId.BUCKET_OF_WATER.id()) || (item2.getID() == ItemId.JUG_OF_WATER.id())) || (item2.getID() == ItemId.BOWL_OF_WATER.id())) && useWater(player, item2, item1)) {
                                                            return null;
                                                        } else
                                                            if (((item1.getID() == ItemId.MOLTEN_GLASS.id()) && (item2.getID() == ItemId.LENS_MOULD.id())) || ((item1.getID() == ItemId.LENS_MOULD.id()) && (item2.getID() == ItemId.MOLTEN_GLASS.id()))) {
                                                                if (Functions.getCurrentLevel(player, Skills.CRAFTING) < 10) {
    ...
    private boolean craftingChecks(final GameObject obj, final Item item, final Player player) {
        ...
        if (!craftingTypeChecks(obj, item, player))
            return false;
        ...
        if ((item.getID() == ItemId.SODA_ASH.id()) || (item.getID() == ItemId.SAND.id())) {
            ...
            if (player.getInventory().countId(ItemId.SODA_ASH.id()) < 1) {
                ...
                if (player.getInventory().countId(ItemId.SAND.id()) < 1) {
    ...
    private boolean craftingTypeChecks(final GameObject obj, final Item item, final Player player) {
        return ((((obj.getID() == 118) || (obj.getID() == 813)) && DataConversions.inArray(itemsFurnance, item.getID())) || ((obj.getID() == 178) && DataConversions.inArray(itemsOven, item.getID()))) || ((obj.getID() == 179) && (item.getID() == ItemId.SOFT_CLAY.id()));
    }
    ...
    private void beginCrafting(final Item item, final Player player) {
        if ((item.getID() == ItemId.SODA_ASH.id()) || (item.getID() == ItemId.SAND.id())) {
            ...
            if (DataConversions.inArray(itemsOven, item.getID())) {
    ...
    private void doGoldJewelry(final Item item, final Player player) {
        ...
        final int[] moulds = new int[]{ ItemId.RING_MOULD.id(), ItemId.NECKLACE_MOULD.id(), ItemId.AMULET_MOULD.id() };
        final int[] gems = new int[]{ ItemId.NOTHING.id(), ItemId.SAPPHIRE.id(), ItemId.EMERALD.id(), ItemId.RUBY.id(), ItemId.DIAMOND.id(), ItemId.DRAGONSTONE.id(), ItemId.OPAL.id() };
        // select gem
        options = new String[]{ "Gold", "Sapphire", "Emerald", "Ruby", "Diamond" };
        if (player.getWorld().getServer().getConfig().MEMBER_WORLD) {
            if (player.getWorld().getServer().getConfig().WANT_EQUIPMENT_TAB) {
                options = new String[]{ "Gold", "Sapphire", "Emerald", "Ruby", "Diamond", "Dragonstone", "Opal" };
            } else {
                options = new String[]{ "Gold", "Sapphire", "Emerald", "Ruby", "Diamond", "Dragonstone" };
            }
        }
        ...
        int gem = Functions.___showMenu(player, options);
            ...
            public void action() {
                ...
                if ((getOwner().getInventory().remove(item) > (-1)) && ((gem == 0) || (getOwner().getInventory().remove(gems[gem], 1) > (-1)))) {
                    ...
                    if (((item.getID() == ItemId.GOLD_BAR_FAMILYCREST.id()) && (gem == 3)) && (type == 0)) {
                        ...
                        if (((item.getID() == ItemId.GOLD_BAR_FAMILYCREST.id()) && (gem == 3)) && (type == 1)) {
    ...
    private void doSilverJewelry(final Item item, final Player player) {
        ...
        reply.set(options[type]);
        int[] moulds = new int[]{ ItemId.HOLY_SYMBOL_MOULD.id(), ItemId.UNHOLY_SYMBOL_MOULD.id() };
        final int[] results = new int[]{ ItemId.UNSTRUNG_HOLY_SYMBOL_OF_SARADOMIN.id(), ItemId.UNSTRUNG_UNHOLY_SYMBOL_OF_ZAMORAK.id() };
        if (player.getInventory().countId(moulds[type]) < 1) {
            player.message(((("You need a " + player.getWorld().getServer().getEntityHandler().getItemDef(moulds[type]).getName()) + " to make a ") + reply.get()) + "!");
            return;
        }
            ...
            public void action() {
                ...
                if (getOwner().getInventory().remove(item) > (-1)) {
                    if (fail) {
                        ...
                        getOwner().playerServerMessage(MessageType.QUEST, ("the " + potteryItem) + " hardens in the oven");
                        getWorld().getServer().getGameEventHandler().add(new SingleEvent(getWorld(), getOwner(), 1800, "Remove Clay From Oven") {
                            @Override
                            public void action() {
                                getOwner().playerServerMessage(MessageType.QUEST, ("You remove a " + result.getDef(getWorld()).getName().toLowerCase()) + " from the oven");
                                getOwner().getInventory().add(result);
                                getOwner().incExp(Skills.CRAFTING, exp, true);
                            }
                        });
    ...
    private void doGlassMaking(final Item item, final Player player) {
        ...
        player.setBatchEvent(new BatchEvent(player.getWorld(), player, player.getWorld().getServer().getConfig().GAME_TICK, "Craft Molten Glass", repeatTimes, false) {
            public void action() {
                if ((getOwner().getInventory().countId(otherItem) < 1) || (getOwner().getInventory().countId(item.getID()) < 1)) {
                    interrupt();
                    return;
                }
                if (getWorld().getServer().getConfig().WANT_FATIGUE) {
                ...
                if ((getOwner().getInventory().remove(otherItem, 1) > (-1)) && (getOwner().getInventory().remove(item) > (-1))) {
        ...
        player.setBatchEvent(new BatchEvent(player.getWorld(), player, 600, "Cut Gem", player.getInventory().countId(gem.getID()), false) {
            ...
            public void action() {
                if (getOwner().getSkills().getLevel(Skills.CRAFTING) < gemDef.getReqLevel()) {
                    ...
                    getOwner().playerServerMessage(MessageType.QUEST, (("you need a crafting level of " + gemDef.getReqLevel()) + " to cut ") + (gem.getDef(getWorld()).getName().contains("ruby") ? "rubies" : gem.getDef(getWorld()).getName().replaceFirst("(?i)uncut ", "") + (pluralize ? "s" : "")));
                    interrupt();
                    return;
                }
                if (getOwner().getInventory().remove(gem.getID(), 1, false) > (-1)) {
                    ...
                    if (DataConversions.inArray(gemsThatFail, gem.getID()) && Formulae.smashGem(gem.getID(), gemDef.getReqLevel(), getOwner().getSkills().getLevel(Skills.CRAFTING))) {
                        ...
                        if (gem.getID() == ItemId.UNCUT_RED_TOPAZ.id()) {
                            ...
                            if (gem.getID() == ItemId.UNCUT_JADE.id()) {
    ...
    private void doGlassBlowing(Player player, final Item pipe, final Item glass) {
        ...
        player.setBatchEvent(new BatchEvent(player.getWorld(), player, 600, "Craft Glass Blowing", player.getInventory().countId(glass.getID()), false) {
            ...
            public void action() {
                if (getOwner().getSkills().getLevel(Skills.CRAFTING) < reqLvl) {
                    getOwner().message((("You need a crafting level of " + reqLvl) + " to make ") + resultGen);
                    interrupt();
                    return;
                }
                if (getWorld().getServer().getConfig().WANT_FATIGUE) {
    ...
    private void useWool(Player player, final Item woolBall, final Item item) {
        ...
        player.setBatchEvent(new BatchEvent(player.getWorld(), player, 600, "Craft String Amulet", woolAmount < amuletAmount ? woolAmount : amuletAmount, false) {
            public void action() {
    ...
    private boolean useWater(Player player, final Item water, final Item item) {
        ...
        if ((item.getID() == ItemId.CLAY.id()) && (water.getID() != ItemId.BOWL_OF_WATER.id())) {
            if ((player.getInventory().remove(water) > (-1)) && (player.getInventory().remove(item) > (-1))) {
                Functions.___message(player, 1200, "You mix the clay and water");
    ...
    public boolean blockInvUseOnItem(Player player, Item item1, Item item2) {
        ...
        if ((item1.getID() == ItemId.CHISEL.id()) && ((gemDef != null) || (gemDef2 != null))) {
            ...
            if ((item2.getID() == ItemId.CHISEL.id()) && ((gemDef != null) || (gemDef2 != null))) {
                return true;
            } else
                if (item1.getID() == ItemId.GLASSBLOWING_PIPE.id()) {
                    return true;
                            ...
                            if (item2.getID() == ItemId.NEEDLE.id()) {
                                ...
                                if (item1.getID() == ItemId.BALL_OF_WOOL.id()) {
                                    ...
                                    if (item2.getID() == ItemId.BALL_OF_WOOL.id()) {
                                        ...
                                        if (((item1.getID() == ItemId.BUCKET_OF_WATER.id()) || (item1.getID() == ItemId.JUG_OF_WATER.id())) && (item2.getID() == ItemId.CLAY.id())) {
                                            return true;
                                        } else
                                            if (((item2.getID() == ItemId.BUCKET_OF_WATER.id()) || (item2.getID() == ItemId.JUG_OF_WATER.id())) && (item1.getID() == ItemId.CLAY.id())) {
                                                return true;
                                            } else
