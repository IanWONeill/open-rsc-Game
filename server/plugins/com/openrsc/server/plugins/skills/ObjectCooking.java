...
public class ObjectCooking implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    ...
    public GameStateEvent onInvUseOnObject(GameObject object, Item item, Player owner) {
        return new GameStateEvent(owner.getWorld(), owner, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    Npc cook = Functions.getNearestNpc(owner, 7, 20);
                    if (((cook != null) && (owner.getQuestStage(Quests.COOKS_ASSISTANT) != (-1))) && (object.getID() == 119)) {
                        cook.face(owner);
                        owner.face(cook);
                        Functions.___npcTalk(owner, cook, "Hey! Who said you could use that?");
                    } else
        ...
        if (object.getID() == 491) {
            if (item.getID() == ItemId.RAW_RAT_MEAT.id()) {
                ...
                if (p.getCache().hasKey("tutorial") && (p.getCache().getInt("tutorial") == 25)) {
                    ...
                    Functions.___message(p, "sometimes you will burn food", "As your cooking level increases this will happen less", "Now speak to the cooking instructor again");
                    p.getCache().set("tutorial", 30);
                } else
                    if (p.getCache().hasKey("tutorial") && (p.getCache().getInt("tutorial") == 30)) {
                        ...
                        Functions.___message(p, "Now speak to the cooking instructor again");
            ...
            return;
        } else// Raw Oomlie Meat (Always burn)

            if (item.getID() == ItemId.RAW_OOMLIE_MEAT.id()) {
                if ((object.getID() == 97) || (object.getID() == 274))
                    Functions.___message(p, 1200, "You cook the meat on the fire...");
                else
                    Functions.___message(p, 1200, "You cook the meat on the stove...");

                Functions.removeItem(p, ItemId.RAW_OOMLIE_MEAT.id(), 1);
                Functions.addItem(p, ItemId.BURNTMEAT.id(), 1);
                Functions.___message(p, 1200, "This meat is too delicate to cook like this.");
                Functions.___message(p, 1200, "Perhaps you can wrap something around it to protect it from the heat.");
            } else// Poison (Hazeel Cult)

                if ((((item.getID() == ItemId.POISON.id()) && (object.getID() == 435)) && (object.getX() == 618)) && (object.getY() == 3453)) {
                    if ((p.getQuestStage(Quests.THE_HAZEEL_CULT) == 3) && p.getCache().hasKey("evil_side")) {
                        Functions.___message(p, "you poor the poison into the hot pot", "the poison desolves into the soup");
                        Functions.removeItem(p, ItemId.POISON.id(), 1);
                        p.updateQuestStage(Quests.THE_HAZEEL_CULT, 4);
                    } else {
                        p.message("nothing interesting happens");
                    }
                } else
                    if (item.getID() == ItemId.UNCOOKED_SWAMP_PASTE.id()) {
                        ...
                        if (item.getID() == ItemId.SEAWEED.id()) {
                            // Seaweed (Glass)
                            cookMethod(p, ItemId.SEAWEED.id(), ItemId.SODA_ASH.id(), true, "You put the seaweed on the " + object.getGameObjectDef().getName().toLowerCase(), "The seaweed burns to ashes");
                        } else
                            if (item.getID() == ItemId.COOKEDMEAT.id()) {
                                // Cooked meat to get burnt meat
                                if (p.getQuestStage(Quests.WITCHS_POTION) != (-1)) {
                                    Functions.showBubble(p, item);
                                    Functions.___message(p, 1800, cookingOnMessage(p, item, object, false));
                                    Functions.removeItem(p, ItemId.COOKEDMEAT.id(), 1);
                                    Functions.addItem(p, ItemId.BURNTMEAT.id(), 1);
                                ...
                                if (p.getSkills().getLevel(Skills.COOKING) < cookingDef.getReqLevel()) {
                                    ...
                                    itemName = (itemName.startsWith("raw ")) ? itemName.substring(4) : itemName.startsWith("uncooked ") ? itemName.substring(9) : itemName;
                                ...
                                p.setBatchEvent(new BatchEvent(p.getWorld(), p, timeToCook, "Cooking on Object", p.getInventory().countId(item.getID()), false) {
                                    ...
                                    public void action() {
                                        ...
                                        if (getOwner().getInventory().remove(item) > (-1)) {
                                            if (((!Formulae.burnFood(getOwner(), item.getID(), getOwner().getSkills().getLevel(Skills.COOKING))) || (item.getID() == ItemId.RAW_LAVA_EEL.id())) || ((item.getID() == ItemId.UNCOOKED_PITTA_BREAD.id()) && (getOwner().getSkills().getLevel(Skills.COOKING) >= 58))) {
    ...
    public boolean blockInvUseOnObject(GameObject obj, Item item, Player player) {
        ...
        Arrays.sort(ids);
        if (((((item.getID() == ItemId.RAW_OOMLIE_MEAT.id()) || (item.getID() == ItemId.SEAWEED.id())) || (item.getID() == ItemId.UNCOOKED_SWAMP_PASTE.id())) || (item.getID() == ItemId.COOKEDMEAT.id())) && (Arrays.binarySearch(ids, obj.getID()) >= 0)) {
            return true;
        }
    ...
    private void cookMethod(Player p, int itemID, int product, boolean hasBubble, String... messages) {
        if (Functions.hasItem(p, itemID, 1)) {
            if (hasBubble)
                Functions.showBubble(p, new Item(itemID));

            p.playSound("cooking");
            Functions.___message(p, messages);
            Functions.removeItem(p, itemID, 1);
            Functions.addItem(p, product, 1);
        } else {
    ...
    private String cookingOnMessage(Player p, Item item, GameObject object, boolean needsOven) {
        String itemName = item.getDef(p.getWorld()).getName().toLowerCase();
        itemName = (itemName.startsWith("raw ")) ? itemName.substring(4) : itemName.contains("pie") ? "pie" : itemName.startsWith("uncooked ") ? itemName.substring(9) : itemName;
        boolean isGenMeat = isGeneralMeat(item);
        if (isGenMeat)
            itemName = "meat";

        String message = ((("You cook the " + itemName) + " on the ") + ((object.getID() == 97) || (object.getID() == 274) ? "fire" : "stove")) + (isGenMeat ? "..." : "");
        if (needsOven) {
    ...
    private boolean isOvenFood(Item item) {
        return DataConversions.inArray(new int[]{ ItemId.BREAD_DOUGH.id()// Bread
    ...
    private boolean isGeneralMeat(Item item) {
        return DataConversions.inArray(new int[]{ ItemId.COOKEDMEAT.id(), ItemId.RAW_CHICKEN.id(), ItemId.RAW_BEAR_MEAT.id(), ItemId.RAW_RAT_MEAT.id(), ItemId.RAW_BEEF.id() }, item.getID());
    }
}
