...
public class Smelting implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    ...
    public GameStateEvent onInvUseOnObject(GameObject obj, Item item, Player p) {
        return new GameStateEvent(p.getWorld(), p, 0, (getClass().getSimpleName() + " ") + Thread.currentThread().getStackTrace()[1].getMethodName()) {
            public void init() {
                addState(0, () -> {
                    if ((obj.getID() == Smelting.FURNACE) && (!DataConversions.inArray(new int[]{ ItemId.GOLD_BAR.id(), ItemId.SILVER_BAR.id(), ItemId.SAND.id(), ItemId.GOLD_BAR_FAMILYCREST.id() }, item.getID()))) {
                        if (item.getID() == ItemId.STEEL_BAR.id()) {
                            if (p.getInventory().hasItemId(ItemId.CANNON_AMMO_MOULD.id())) {
                                if (Functions.getCurrentLevel(p, Skills.SMITHING) < 30) {
                                ...
                                Functions.showBubble(p, new Item(ItemId.MULTI_CANNON_BALL.id(), 1));
                                ...
                                Functions.___message(p, messagedelay, "you heat the steel bar into a liquid state", "and pour it into your cannon ball mould", "you then leave it to cool for a short while");
                                p.setBatchEvent(new BatchEvent(p.getWorld(), p, delay, "Smelting", p.getInventory().countId(item.getID()), false) {
                                    ...
                                    public void action() {
                                        ...
                                        if (!isCompleted()) {
                                            ...
                                            Functions.showBubble(getOwner(), new Item(ItemId.MULTI_CANNON_BALL.id(), 1));
                                        }
                                        if (Functions.getCurrentLevel(getOwner(), Skills.SMITHING) < 30) {
    ...
    private void handleRegularSmelting(final Item item, Player p, final GameObject obj) {
        if (!Functions.inArray(item.getID(), Smelting.Smelt.ADAMANTITE_ORE.getID(), Smelting.Smelt.COAL.getID(), Smelting.Smelt.COPPER_ORE.getID(), Smelting.Smelt.IRON_ORE.getID(), Smelting.Smelt.GOLD.getID(), Smelting.Smelt.MITHRIL_ORE.getID(), Smelting.Smelt.RUNITE_ORE.getID(), Smelting.Smelt.SILVER.getID(), Smelting.Smelt.TIN_ORE.getID(), ItemId.GOLD_FAMILYCREST.id())) {
            p.message("Nothing interesting happens");
            return;
        }
        String formattedName = item.getDef(p.getWorld()).getName().toUpperCase().replaceAll(" ", "_");
        Smelting.Smelt smelt;
        if (((item.getID() == Smelting.Smelt.IRON_ORE.getID()) && (Functions.getCurrentLevel(p, Skills.SMITHING) >= 30)) && (p.getInventory().countId(Smelting.Smelt.COAL.getID()) >= 2)) {
            String coalChange = p.getWorld().getServer().getEntityHandler().getItemDef(Smelting.Smelt.COAL.getID()).getName().toUpperCase();
            smelt = Smelting.Smelt.valueOf(coalChange);
        } else {
            smelt = Smelting.Smelt.valueOf(formattedName);
        }
        ...
        if (Functions.getCurrentLevel(p, Skills.SMITHING) < smelt.getRequiredLevel()) {
        ...
        if ((p.getInventory().countId(smelt.getReqOreId()) < smelt.getReqOreAmount()) || ((p.getInventory().countId(smelt.getID()) < smelt.getOreAmount()) && (smelt.getReqOreAmount() != (-1)))) {
            if ((smelt.getID() == Smelting.Smelt.TIN_ORE.getID()) || (item.getID() == Smelting.Smelt.COPPER_ORE.getID())) {
                p.playerServerMessage(MessageType.QUEST, ("You also need some " + (item.getID() == Smelting.Smelt.TIN_ORE.getID() ? "copper" : "tin")) + " to make bronze");
                return;
            }
            if ((smelt.getID() == Smelting.Smelt.COAL.getID()) && ((p.getInventory().countId(Smelting.Smelt.IRON_ORE.getID()) < 1) || (p.getInventory().countId(Smelting.Smelt.COAL.getID()) <= 1))) {
                p.playerServerMessage(MessageType.QUEST, "You need 1 iron-ore and 2 coal to make steel");
                return;
            } else {
                p.playerServerMessage(MessageType.QUEST, (((("You need " + smelt.getReqOreAmount()) + " heaps of ") + p.getWorld().getServer().getEntityHandler().getItemDef(smelt.getReqOreId()).getName().toLowerCase()) + " to smelt ") + item.getDef(p.getWorld()).getName().toLowerCase().replaceAll("ore", ""));
                return;
            }
        }
        ...
        p.setBatchEvent(new BatchEvent(p.getWorld(), p, 1800, "Smelt", Formulae.getRepeatTimes(p, Skills.SMITHING), false) {
            ...
            public void action() {
                ...
                if (Functions.getCurrentLevel(getOwner(), Skills.SMITHING) < smelt.getRequiredLevel()) {
                ...
                if ((getOwner().getInventory().countId(smelt.getReqOreId()) < smelt.getReqOreAmount()) || ((getOwner().getInventory().countId(smelt.getID()) < smelt.getOreAmount()) && (smelt.getReqOreAmount() != (-1)))) {
                    if ((smelt.getID() == Smelting.Smelt.COAL.getID()) && ((getOwner().getInventory().countId(Smelting.Smelt.IRON_ORE.getID()) < 1) || (getOwner().getInventory().countId(Smelting.Smelt.COAL.getID()) <= 1))) {
                        getOwner().playerServerMessage(MessageType.QUEST, "You need 1 iron-ore and 2 coal to make steel");
                        interrupt();
                        return;
                    }
                    if ((smelt.getID() == Smelting.Smelt.TIN_ORE.getID()) || (item.getID() == Smelting.Smelt.COPPER_ORE.getID())) {
                        getOwner().playerServerMessage(MessageType.QUEST, ("You also need some " + (item.getID() == Smelting.Smelt.TIN_ORE.getID() ? "copper" : "tin")) + " to make bronze");
                        interrupt();
                        return;
                    } else {
                        getOwner().playerServerMessage(MessageType.QUEST, (((("You need " + smelt.getReqOreAmount()) + " heaps of ") + getWorld().getServer().getEntityHandler().getItemDef(smelt.getReqOreId()).getName().toLowerCase()) + " to smelt ") + item.getDef(getWorld()).getName().toLowerCase().replaceAll("ore", ""));
                        interrupt();
                        return;
                    }
                }
                ...
                if (getOwner().getInventory().countId(item.getID()) > 0) {
                    if (item.getID() == ItemId.GOLD_FAMILYCREST.id())
                        Functions.removeItem(getOwner(), ItemId.GOLD_FAMILYCREST.id(), 1);
                    else
                    ...
                    if ((smelt.getID() == Smelting.Smelt.IRON_ORE.getID()) && (DataConversions.random(0, 1) == 1)) {
                        if (Functions.isWielding(getOwner(), ItemId.RING_OF_FORGING.id())) {
                            getOwner().message("Your ring of forging shines brightly");
                            Functions.addItem(getOwner(), smelt.getSmeltBarId(), 1);
                        ...
                        if (item.getID() == ItemId.GOLD_FAMILYCREST.id())
                            Functions.addItem(getOwner(), ItemId.GOLD_BAR_FAMILYCREST.id(), 1);
                        else
                            Functions.addItem(getOwner(), smelt.getSmeltBarId(), 1);
    ...
    private String smeltString(World world, Smelting.Smelt smelt, Item item) {
        String message = null;
        if (smelt.getSmeltBarId() == ItemId.BRONZE_BAR.id()) {
            message = "You smelt the copper and tin together in the furnace";
        } else
            if (((smelt.getSmeltBarId() == ItemId.MITHRIL_BAR.id()) || (smelt.getSmeltBarId() == ItemId.ADAMANTITE_BAR.id())) || (smelt.getSmeltBarId() == ItemId.RUNITE_BAR.id())) {
                ...
                if (smelt.getSmeltBarId() == ItemId.STEEL_BAR.id()) {
                    ...
                    if (smelt.getSmeltBarId() == ItemId.IRON_BAR.id()) {
                        ...
                        if (((smelt.getSmeltBarId() == ItemId.SILVER_BAR.id()) || (smelt.getSmeltBarId() == ItemId.GOLD_BAR.id())) || (smelt.getSmeltBarId() == ItemId.GOLD_BAR_FAMILYCREST.id())) {
    ...
    public boolean blockInvUseOnObject(GameObject obj, Item item, Player player) {
        return (obj.getID() == Smelting.FURNACE) && (!DataConversions.inArray(new int[]{ ItemId.GOLD_BAR.id(), ItemId.SILVER_BAR.id(), ItemId.SODA_ASH.id(), ItemId.SAND.id(), ItemId.GOLD_BAR_FAMILYCREST.id() }, item.getID()));
    }
