...
public class Smithing implements InvUseOnObjectListener , InvUseOnObjectExecutiveListener {
    ...
    private boolean allowDorics(Player player) {
        if (player.getQuestStage(Quests.DORICS_QUEST) > (-1)) {
            Npc doric = player.getWorld().getNpc(144, 323, 327, 487, 492, true);
    ...
    private boolean smithingChecks(final GameObject obj, final Item item, final Player player) {
        ...
        if (player.getSkills().getLevel(Skills.SMITHING) < minSmithingLevel) {
            player.message((("You need at least level " + minSmithingLevel) + " smithing to work with ") + item.getDef(player.getWorld()).getName().toLowerCase().replaceAll("bar", ""));
            return false;
        }
    ...
    private void attemptDragonSquareCombine(Item item, Player player) {
        ...
        } else// non-kosher this message

            if ((player.getInventory().countId(ItemId.RIGHT_HALF_DRAGON_SQUARE_SHIELD.id()) < 1) || (player.getInventory().countId(ItemId.LEFT_HALF_DRAGON_SQUARE_SHIELD.id()) < 1)) {
                player.message("You need the two shield halves to repair the shield.");
            } else {
                Functions.___message(player, 1200, "You set to work trying to fix the ancient shield.", "You hammer long and hard and use all of your skill.", "Eventually, it is ready...", "You have repaired the Dragon Square Shield.");
    ...
    private void handleGoldSmithing(Player player) {
        int goldOption = Functions.___showMenu(player, "Golden bowl.", "Cancel");
        if (player.isBusy()) {
            return;
        }
        if (goldOption == 0) {
            Functions.___message(player, "You hammer the metal...");
        ...
        player.setBatchEvent(new BatchEvent(player.getWorld(), player, 600, "Smithing", makeCount, false) {
            ...
            public void action() {
                if (getOwner().getSkills().getLevel(Skills.SMITHING) < def.getRequiredLevel()) {
                    getOwner().message(("You need to be at least level " + def.getRequiredLevel()) + " smithing to do that");
                    interrupt();
                    return;
                }
                ...
                if (getWorld().getServer().getEntityHandler().getItemDef(def.getItemID()).isStackable()) {
                    getOwner().playerServerMessage(MessageType.QUEST, (("You hammer the metal and make " + def.getAmount()) + " ") + getWorld().getServer().getEntityHandler().getItemDef(def.getItemID()).getName().toLowerCase());
                    getOwner().getInventory().add(new Item(def.getItemID(), def.getAmount()));
                } else {
                    getOwner().playerServerMessage(MessageType.QUEST, "You hammer the metal and make a " + getWorld().getServer().getEntityHandler().getItemDef(def.getItemID()).getName().toLowerCase());
    ...
    private int firstMenu(Item item, Player player) {
        ...
        if (item.getID() == ItemId.STEEL_BAR.id()) {
            option = Functions.___showMenu(player, "Make Weapon", "Make Armour", "Make Missile Heads", "Make Steel Nails", "Cancel");
            // Cancel
            if (option == 4)
                return -1;
        ...
        if (item.getID() == ItemId.BRONZE_BAR.id()) {
            option = Functions.___showMenu(player, "Make Weapon", "Make Armour", "Make Missile Heads", "Make Craft Item", "Cancel");
            // Cancel
            if (option == 4)
                return -1;
        ...
        // Any other bar.
        option = Functions.___showMenu(player, "Make Weapon", "Make Armour", "Make Missile Heads", "Cancel");
        if (option == 3)
            return -1;
    ...
    private int secondMenu(Item item, Player player, int firstType) {
        ...
        if (firstType == 0) {
            ...
            return Functions.___showMenu(player, "Dagger", "Throwing Knife", "Sword", "Axe", "Mace");
        }
        offset += 5;
        // Armour
        if (firstType == 1) {
            player.message("Choose a type of armour to make");
            int option = Functions.___showMenu(player, "Helmet", "Shield", "Armour");
            // Cancel
            if (option < 0)
                return -1;
        ...
        if (firstType == 2) {
            ...
            if ((player.getQuestStage(Quests.TOURIST_TRAP) >= 8) || (player.getQuestStage(Quests.TOURIST_TRAP) == (-1))) {
                option = Functions.___showMenu(player, "Make Arrow Heads.", "Forge Dart Tips.", "Cancel.");
                // Cancel
                if (option == 2)
                    return -1;

            } else // Before Tourist Trap
            {
                option = Functions.___showMenu(player, "Make Arrow Heads.", "Cancel.");
                // Cancel
                if (option == 2)
                    return -1;
    ...
    private void makeWire(Item item, Player player) {
        ...
        int bronzeWireOption = Functions.___showMenu(player, "Bronze Wire(1 bar)", "Cancel");
        if (player.isBusy()) {
            return;
        }
    ...
    private int chooseItem(Player player, int secondType) {
        ...
        if (secondType == 0)
            ...
            if (secondType == 1) {
                ...
                if (secondType == 2)
                    return swordChoice(player);
                else// Axe

                    if (secondType == 3)
                        return axeChoice(player);
                    else// Mace
                        ...
                        if (secondType == 4)
                            ...
                            if (secondType == 5)
                                return helmetChoice(player);
                            else// Shield

                                if (secondType == 6)
                                    return shieldChoice(player);
                                else// Armour

                                    if (secondType == 7)
                                        return armourChoice(player);
                                    else// Arrowheads
    ...
    private int swordChoice(Player player) {
        ...
        int option = Functions.___showMenu(player, "Short Sword", "Long Sword (2 bars)", "Scimitar (2 bars)", "2-handed Sword (3 bars)");
        if (option == 0)
            return 2;
        // 2-handed Sword
        else// Short Sword
    ...
    private int axeChoice(Player player) {
        ...
        int option = Functions.___showMenu(player, "Hatchet", "Battle Axe (3 bars)");
        if (option == 0)
            return 6;
        // Battle Axe
        else// Hatchet
    ...
    private int helmetChoice(Player player) {
        ...
        int option = Functions.___showMenu(player, "Medium Helmet", "Large Helmet (2 bars)");
        if (option == 0)
            return 10;
        // Large Helmet
        else// Medium Helmet
    ...
    private int shieldChoice(Player player) {
        ...
        int option = Functions.___showMenu(player, "Square Shield (2 bars)", "Kite Shield (3 bars)");
        if (option == 0)
            return 12;
        // Kite Shield
        else// Square Shield
    ...
    private int armourChoice(Player player) {
        ...
        int option = Functions.___showMenu(player, "Chain Mail Body (3 bars)", "Plate Mail Body (5 bars)", "Plate Mail Legs (3 bars)", "Plated Skirt (3 bars)");
        if (option == 0)
            return 14;
        // Plated Skirt
        else// Chain Mail Body

            if (option == 1)
                return 15;
            else// Plate Mail Body

                if (option == 2)
                    return 16;
                else// Plate Mail Legs
    ...
    private int getCount(ItemSmithingDef def, Item item, Player player) {
        ...
        if (player.getWorld().getServer().getConfig().BATCH_PROGRESSION) {
            String[] options = new String[]{ "Make 1", "Make 5", "Make 10", "Make All" };
            count = Functions.___showMenu(player, options);
            if (count == (-1)) {
                return -1;
            }
            int maximumMakeCount = player.getInventory().countId(item.getID()) / def.getRequiredBars();
            return count != 3 ? Integer.parseInt(options[count].replaceAll("Make ", "")) : maximumMakeCount;
        }
        return count;
    }
    ...
    public int getSmithingExp(int barID, int barCount) {
        int[] exps = new int[]{ 50, 100, 150, 200, 250, 300 };
