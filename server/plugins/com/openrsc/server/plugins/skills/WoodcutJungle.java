...
public class WoodcutJungle implements ObjectActionListener , WallObjectActionListener , ObjectActionExecutiveListener , WallObjectActionExecutiveListener {
    private static int[] JUNGLE_TREES = new int[]{ 1086, 1100, 1099, 1092, 1091 };
    ...
    public boolean blockObjectAction(GameObject obj, String command, Player p) {
        if (Functions.inArray(obj.getID(), WoodcutJungle.JUNGLE_TREES)) {
            return true;
        }
        if (obj.getID() == WoodcutJungle.JUNGLE_TREE_STUMP) {
            return true;
        }
        return false;
    }
    ...
    private void handleJungleWoodcut(GameObject obj, Player p) {
        if (p.getFatigue() >= p.MAX_FATIGUE) {
            p.playerServerMessage(MessageType.QUEST, "You are too tired to cut the " + (obj.getID() == WoodcutJungle.JUNGLE_VINE ? "jungle vines" : "tree"));
            // Shilo side of the jungle
            if (p.getY() < 866) {
                return;
            } else
                if (p.getY() >= 866) {
                    // Khazari side, force out.
        ...
        if (!Functions.hasItem(p, ItemId.MACHETTE.id())) {
            Functions.___message(p, 1900, "This jungle is very thick, you'll need a machette to cut through.");
            return;
        }
        int axeId = -1;
        if (obj.getID() != WoodcutJungle.JUNGLE_VINE) {
            for (final int a : Formulae.woodcuttingAxeIDs) {
                if (Functions.hasItem(p, a)) {
                    axeId = a;
                    break;
                }
            }
        ...
        p.setBusy(true);
        Functions.showBubble(p, new Item(axeId));
        p.playerServerMessage(MessageType.QUEST, ((("You swing your " + p.getWorld().getServer().getEntityHandler().getItemDef(axeId).getName().toLowerCase()) + " at the ") + (obj.getID() == WoodcutJungle.JUNGLE_VINE ? "jungle vines" : "tree")) + "...");
        ...
        if (force || getLog(50, p.getSkills().getLevel(Skills.WOODCUT), axeId)) {
            ...
            if ((jungleObject != null) && (jungleObject.getID() == obj.getID())) {
                if (obj.getID() == WoodcutJungle.JUNGLE_VINE) {
                    ...
                    if (!force)
                        Functions.___message(p, 1200, "You hack your way through the jungle.");

                } else {
                    p.getWorld().replaceGameObject(obj, new GameObject(obj.getWorld(), obj.getLocation(), WoodcutJungle.JUNGLE_TREE_STUMP, obj.getDirection(), obj.getType()));
            ...
            p.playerServerMessage(MessageType.QUEST, "You slip and fail to hit the " + (obj.getID() == WoodcutJungle.JUNGLE_VINE ? "jungle vines" : "tree"));
        }
        p.setBusy(false);
    }
    ...
    public boolean blockWallObjectAction(GameObject obj, Integer click, Player player) {
        return obj.getID() == WoodcutJungle.JUNGLE_VINE;
    }
